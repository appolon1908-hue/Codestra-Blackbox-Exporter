#!/usr/bin/env python3
"""Build the deterministic Blackbox Exporter configuration archive."""
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "dist/blackbox-exporter-config.tar.gz")
    args = parser.parse_args(); manifest = json.loads(MANIFEST.read_text()); paths = []
    for relative, expected in sorted(manifest["files"].items()):
        path = ROOT / relative
        if digest(path) != expected.removeprefix("sha256:"): raise SystemExit(f"checksum mismatch: {relative}")
        paths.append(path)
